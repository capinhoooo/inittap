...
function parseTxError(err: unknown): string {
  ...
  if (
    ...
    msg.includes('AgentWalletAlreadyRegistered')
  )
    return 'This wallet is already registered as an agent'
  if (msg.includes('sequence mismatch'))
    return 'Sequence mismatch (stale nonce). Please try again.'
  if (msg.includes('Reverted'))
    return 'Transaction reverted on-chain. The wallet may already be registered.'
  if (msg.includes('execution reverted')) {
    const revertMatch = msg.match(/reason:\s*(.+?)(?:\n|$)/)
    return revertMatch
      ? `Reverted: ${revertMatch[1]}`
      : `Contract reverted: ${msg.slice(0, 120)}`
  }
  return `Transaction failed: ${msg.slice(0, 150)}`
}
...
function RegisterAgentButtonConnected() {
  ...
  async function handleRegister() {
    ...
    try {
      checksummedWallet = getAddress(agentWallet)
    } catch {
      setWalletError('Invalid address checksum')
      return
    }

    // Pre-flight: check if wallet is already registered
    try {
      const client = createPublicClient({
        transport: http(config.chain.rpcUrl),
      })
      const existingId = await client.readContract({
        address: getAddress(config.contracts.agentRegistry),
      ...
      if (existingId > 0n) {
        setWalletError(
          `This wallet is already registered as Agent #${existingId}`,
        )
        return
      }
    } catch (e) {
      console.warn('[RegisterAgent] Pre-flight check failed, proceeding:', e)
    }
    ...
    const calldata = encodeFunctionData({
      ...
      args: [checksummedWallet, strategyURI, feeBps],
    })
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      try {
        console.log(
          `[RegisterAgent] Attempt ${attempt + 1}/${MAX_RETRIES + 1}, sending tx...`,
        )
        ...
        console.log('[RegisterAgent] Success:', result)
        ...
        console.error(`[RegisterAgent] Attempt ${attempt + 1} error:`, errMsg)

        if (errMsg.includes('sequence mismatch') && attempt < MAX_RETRIES) {
          console.log(`[RegisterAgent] Sequence mismatch, retrying in 1.5s...`)
...
function CopyTradeModal({
  ...
  const { isConnected, openConnect, requestTxBlock, initiaAddress } =
    useInterwovenKit()
  const [state, setState] = React.useState<DepositState>({ phase: 'input' })
  const [amount, setAmount] = React.useState(String(MIN_DEPOSIT))
  ...
  async function handleDeposit() {
    ...
    if (!amount || isNaN(val) || amount.includes('e')) {
      setAmountError('Enter a valid number')
      return
    }
    if (val < MIN_DEPOSIT) {
      setAmountError(`Minimum deposit is ${MIN_DEPOSIT} INIT`)
      return
    }
    if (!initiaAddress) return
    setAmountError(null)
        ...
        >
          ...
          >
            {state.phase === 'success' ? (
              ...
              <div className="bg-[#1f2228] border border-white/10 rounded-xl p-5">
                <p className="font-sans text-[13px] text-white/60 mb-4 tracking-[-0.01em]">
                  Deposit to copy Agent #{agentId}'s trades
                </p>
      ...
      >
        ...
        <div className="flex gap-2 mt-auto">
          ...
          <motion.button
            ...
            className="flex-1 bg-white text-[#1f2228] font-mono text-[12px] uppercase tracking-[0.06em] py-2 rounded-lg hover:bg-white/90 transition-colors duration-200"
          >
            Copy Trade
          </motion.button>
        </div>
