...
contract VipScore {
    ...
    mapping(uint64 => mapping(address => Score)) public scores;
    mapping(uint64 => mapping(uint64 => address)) private scoreKeys;
    mapping(uint64 => uint64) private scoreLength;
    ...
    function increaseScore(uint64 stage, address addr, uint64 amount) external onlyAllower {
        if (addr == address(0x0)) { return; }
        if (stages[stage].stage == 0) { revert StageNotFound(stage); }
        if (stages[stage].isFinalized) { revert StageFinalized(stage); }
    ...
    function createStage(uint64 stage) private {
        if (stage == 0) { revert ZeroStage({}); }
        if (stages[stage].stage != 0) { return; }
        StageInfo storage stageInfo = stages[stage];
